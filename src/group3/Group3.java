...
public class Group3 extends JFrame {
	...
	ArrayList<String> friendList = new ArrayList(); // List of friends added by
													// user
	ArrayList<String> groupFriendList = new ArrayList(); // List of friends
															// within the
															// current group
															// chat (Who is
															// currently online)
	ArrayList<String> ownCreatedGroupList = new ArrayList(); // List of chat
																// group created
																// by this user
	ArrayList<String> groupList = new ArrayList(); // List of group added by
													// user (Inclusive of own
													// created groups)

	boolean broadcastCreateGroup = false; // Use while in the process of
											// creating a new chat group
	boolean chatGroupThreadCreated = false; // Use to check whether any chat
											// group thread is created
	...
	public Group3() {
		...
		btnRegister.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (tentativeName.equals("")) {
					...
					try {
						// Broadcast tentative user name to check any other user
						// has the same name
						...
						multicastBroadcastSocket.setSoTimeout(3000); // Set
																		// timeout
																		// to 3
																		// seconds,
																		// so
						...
						try {
							...
							multicastBroadcastSocket.setSoTimeout(0); // Clear
																		// timeout
						...
						// This section run when user name does not exist
							...
							JOptionPane.showMessageDialog(null,
									registeredName + ", you have been successfully registered!");
										...
										try {
											...
											if (broadcastCreateGroup) {
												...
												// group does not exist
													...
													// Generate a randomise IP
													// address of a new chat
													// group
													Random rand = new Random();
													String groupIP = "235.1" + "." + (rand.nextInt(254) + 1) + "."
															+ (rand.nextInt(254) + 3);
													System.out.println("New IP address: " + groupIP
															+ " generated for group " + groupName);

													// Add group into own
													// created list
													ownCreatedGroupList.add(groupName + "|" + groupIP);

													// Add group into current
													// active group (To be
													// display in the drop down
													// list)
													groupList.add(groupName + "|" + groupIP);
													UpdateGroupList(); // Update
																		// group
																		// drop
																		// down
																		// list
													...
													JOptionPane.showMessageDialog(null,
															groupName + " chat group has been successfully created!");
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (tentativeFriendName.equals("")) {
					...
					// Check if input friend name is the user name
						...
						try {
							// Broadcast friend request over the network, only
							// the specific friend will process the request
							String friendName = "CheckFriendName|" + tentativeFriendName + "|" + registeredName;
							byte[] sendBuf = friendName.getBytes();
							DatagramPacket dgpSend = new DatagramPacket(sendBuf, sendBuf.length,
									multicastBroadcastGroup, PORT);
							multicastBroadcastSocket.send(dgpSend);
							txtFriend.setText("");
						} catch (IOException ex) {
							ex.printStackTrace();
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (txtFriend.getText().trim().equals("")) {
					...
					if (friendList.contains(txtFriend.getText())) {
						...
						JOptionPane.showMessageDialog(null,
								txtFriend.getText() + " doesnt exists in your friend list!");
		...
		btnAddGroup.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (tentativeGroupName.equals("")) {
					...
					// Check whether the chat group is already exist in the user
					// group list (To prevent duplication)
					if (CheckGroupListExist(tentativeGroupName)) {
						...
						try {
							// Broadcast group name to everyone in the network
							// to check any user own the name of the tentative
							// group name (To prevent duplication)
		...
		btnAddGroupFriend.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					String joinGroupRequest = friendName + "|JoinGroupRequest|" + registeredName + "|" + groupName + "|"
							+ GetGroupIP(groupName);
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (!chatGroupThreadCreated) {
						// Create a new thread to keep listening for packets
						// from the group
						new Thread(new Runnable() {
							@Override
							public void run() {
						...
						try {
							// Broadcast to know who are current online in this
							// particular chat group
							String whoIsThere = "WhoIsThere|" + registeredName;
		...
		btnPrivateChat.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					// Send request to start a private chat with the selected
					// friend
	...
	// This function handle all action need to perform based on the message
	// action header receive from the broadcast group
	public void DoAction(String message) {
		String action = message.substring(0, message.indexOf("|")); // Retrieve
																	// action

		message = message.substring(message.indexOf("|") + 1, message.length()); // Discard
																					// action
																					// string
		...
		// Action to accept friend request
			...
			String nameToCheck = message.substring(0, message.indexOf("|")); // Name
																				// being
																				// request
		...
		// Action when receive reply from previous request
			...
			String replyAction = message.substring(0, message.indexOf("|")); // Retrieve
																				// reply
																				// action
			message = message.substring(message.indexOf("|") + 1, message.length());
			...
			// When a friend request to join a group chat
				...
				int dialogResult = JOptionPane.showConfirmDialog(null,
						friendName + " would like you to join " + groupName + " chat group!", "Join Chat Group",
						dialogButton);

				// Add chat group name into the group chat drop down list when
			...
			// Action for accepting a group chat
				...
				try {
					...
					int dialogResult = JOptionPane.showConfirmDialog(null,
							friendName + " would like to have a private chat with you now. Do you acccept?",
							"Private Chat Request", dialogButton);
					...
					if (dialogResult == 0) {
						...
						String replyPrivateChatRequest = friendName + "|PrivateChatAccepted|" + registeredName + "|"
								+ message;
			...
			// Action for friend accepted private chat session
				...
				try {
					...
					// Send leaving message to inform other user of leaving the
					// previous chat
					SendLeavingMessage();
	...
	// Function to check if input group name exist inside own created group chat
	// list
	...
	public void DoMessageAction(String message) {
		...
		// Receive reply from request
			...
			// Received reply from who is online message for the current chat
			// group
	...
	public void SendLeavingMessage() {
		if (multicastChatGroup != null) {
			// Broadcast to other member that this user is leaving the current
			// group chat
			try {
				String msg = txtMessage.getText();
