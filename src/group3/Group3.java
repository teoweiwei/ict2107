...
public class Group3 extends JFrame {
	...
	public void DoAction(String message) {
		...
		if (friendAdd = true) {
			try {
				byte receiveBuf[] = new byte[1000];
				DatagramPacket dgpReceived2 = new DatagramPacket(receiveBuf, receiveBuf.length);
				multicastBroadcastSocket.receive(dgpReceived2); // discard own
																// broadcast
																// message

				System.out.println("Before Try");
				multicastBroadcastSocket.setSoTimeout(3000); // 3 seconds
																// timeout
				try {
					multicastBroadcastSocket.receive(dgpReceived2);
					multicastBroadcastSocket.setSoTimeout(0); // Disable time
																// out
					System.out.println("received");

					byte[] receivedData = dgpReceived2.getData();
					int length = dgpReceived2.getLength();
					...
					if (receivedMessage.equals("FriendExists|")) {
						...
						// System.out.println(tentativeName + " already
						// taken!");
						// txtUserName.setText("The name " + tentativeName + "
						// is already taken!");
					}

				} catch (SocketTimeoutException ex) {
					// registeredName = tentativeName;
					System.out.println(txtFriend.getText() + " does not exists");
					// btnRegister.setEnabled(false);
					multicastBroadcastSocket.setSoTimeout(0); // Disable time
																// out
